...
pub fn render_markdown() -> String {
    ...
    out.push_str("Proceed with `rskrb5` as the high-level pure-Rust implementation while reusing permissively licensed ASN.1/data-type crates where they pass gokrb5 vectors. Candidate crates remain useful dependencies or references, but none currently supplies a clean gokrb5-equivalent client/service/SPNEGO API. With RFC3962/RFC8009 AES, DES3-CBC-SHA1-KD, and RC4-HMAC coverage, ChangePasswdData builders and KRB-PRIV payload construction, generic AP-REQ construction, direct AS-service login helpers, complete kpasswd request assembly with generated reply keys, kpasswd AP-REP validation, a high-level Tokio password-change helper with initial kadmin/changepw ticket acquisition and credential update, per-enctype keytab login/service-ticket integration, assumed preauthentication, KRB-ERROR surfacing, auto UDP/TCP KDC fallback with Docker AS/TGS and gated kpasswd change/restore coverage, configured/DNS SRV kpasswd transport and typed kpasswd request/reply/result exchanges, GSSAPI MIC plus sealed/unsealed Wrap vectors, SPNEGO client AP-REQ/header generation, config/DNS-backed Tokio KDC discovery, cross-realm TGS referral following, configuration validation, TGT/session renewal, affirm-login reuse, destroy semantics, high-level Tokio client cache JSON/lookup/removal, ccache write-back, HTTP/Tower adapters, an Axum Negotiate example, and PAC credentials, S4U delegation, device info, and compressed/uncompressed client/device claims in place, the immediate implementation work is deeper client/session edge-case coverage.\n");
    out
}
