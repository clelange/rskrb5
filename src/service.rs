...
//!
//! This module is the first service-flow slice: it validates Kerberos AP-REQ
//! messages by decoding the request, decrypting the service ticket from a
//! keytab, decrypting the authenticator with the ticket session key, and
//! enforcing the core identity, ticket-time, clock-skew, address, and replay
//! checks from gokrb5's service tests.
...
impl<'a> ServiceValidator<'a> {
    ...
    pub fn validate_ap_req(&mut self, bytes: &[u8]) -> Result<ValidatedApReq, Error> {
        ...
        let authenticator_time = system_time_from_kerberos_time(&authenticator.ctime)?
