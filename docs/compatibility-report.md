...
Proceed with `rskrb5` as the high-level pure-Rust implementation while reusing permissively licensed ASN.1/data-type crates where they pass gokrb5 vectors. Candidate crates remain useful dependencies or references, but none currently supplies a clean gokrb5-equivalent client/service/SPNEGO API. With RFC3962/RFC8009 AES, DES3-CBC-SHA1-KD, and RC4-HMAC coverage, ChangePasswdData builders and KRB-PRIV payload construction, generic AP-REQ construction, complete kpasswd request assembly with generated reply keys, a high-level Tokio password-change helper, per-enctype keytab login/service-ticket integration, assumed preauthentication, KRB-ERROR surfacing, auto UDP/TCP KDC fallback with Docker AS/TGS coverage, configured kpasswd transport and typed kpasswd request/reply/result exchanges, GSSAPI MIC plus sealed/unsealed Wrap vectors, SPNEGO client AP-REQ/header generation, config/DNS-backed Tokio KDC discovery, cross-realm TGS referral following, TGT/session renewal, high-level Tokio client cache JSON, ccache write-back, HTTP/Tower adapters, an Axum Negotiate example, and PAC credentials, S4U delegation, device info, and compressed/uncompressed client/device claims in place, the immediate implementation work is deeper client/session edge-case coverage.
