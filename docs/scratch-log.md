...
- Open failure: `TEST_KPASSWD=1` changes the Docker test password but fails reply processing with `Crypto(IntegrityCheckFailed)` before restoration; restored `testuser1` to `passwordvalue` manually and verified normal AS login succeeds.
