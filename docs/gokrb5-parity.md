...
| Active Directory TESTAD integration | blocked-on-lab | active-directory, PAC, client, service | GitHub Actions run [`28117082548`](https://github.com/clelange/rskrb5/actions/runs/28117082548) reached hosted `ubuntu-latest` and decoded the four keytab secrets; the four endpoint secrets are still absent. | Set reachable `TEST_AD_*_ADDR` secrets for USER/RESOURCE KDC and admin endpoints, then run `scripts/check-github-ad-gate.py --dispatch`. |
